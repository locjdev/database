...
WITH c3 AS (
			SELECT * 
            FROM question
            WHERE MONTH(created_date) = MONTH(CURRENT_DATE) && YEAR(created_date) = YEAR(CURRENT_DATE)
            )
...
BEGIN
    ...
    FROM c4 WHERE question_count = (SELECT MAX(question_count) FROM c4);
    RETURN v_type_id;
END $$
DELIMITER ;
...
DELIMITER $$
    ...
    BEGIN
        ...
        -- username sẽ giống email nhưng bỏ @
        SELECT SUBSTRING_INDEX(in_email, "@" , 1) INTO v_username;
        
        -- position mặc định là developer
        SELECT position_id INTO v_position_id
        FROM position
        WHERE position_name = "Dev";
        
        -- department_id sẽ được cho vào phòng chờ
        SELECT department_id INTO v_department_id
        FROM department
...
CALL sp_07("Võ Duy Lộc", "Voduyloc@gmail.com");
